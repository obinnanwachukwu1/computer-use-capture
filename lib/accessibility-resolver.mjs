...
export function resolveAccessibilityTarget({
  ...
  // A direct drag already has factual pointer geometry, but that path says
  // nothing about the stable surface being manipulated. Preserve the path as
  // authoritative while allowing AX to contribute a separate interaction
  // container for editorial framing.
  // Stable containing geometry is useful for any pointer interaction. The
  // clicked control remains the factual target; this separate candidate says
  // which persistent surface may own the response. It is evidence, never a
  // role-to-camera rule.
  const wantsInteractionContext = event.action === "drag" || event.action === "click";
  ...
  if (!needsElementTarget
      && !wantsInteractionContext
      && !["type_text", "set_value", "select_text"].includes(event.action)) {
  ...
  // Take Computer Use's requested identity at its word when the last complete
  // pre-action AX tree independently corroborates it. Opening a popover or
  // dialog commonly transfers focus to a newly revealed descendant; that
  // post-action focus describes the result, not the control that was clicked.
  // Result focus remains the fallback when the requested identity cannot be
  // corroborated (for example, when Computer Use acted from a newer tree than
  // the native watcher had observed).
  // Reprocessing an already-normalized timeline should be able to enrich it
  // with newer evidence contracts without needing the original private
  // Computer Use envelope again.
  const requestedTarget = event.accessibilityTarget ?? event.semanticTarget;
  const requestedMatch = requestedTarget
    ? findIdentityMatch(preActionCandidates, requestedTarget, eventMs)
    : undefined;
  const target = requestedMatch ? requestedTarget : (event.postActionFocus ?? requestedTarget);
  let match = requestedMatch ?? findIdentityMatch(candidates, target, eventMs);
  ...
  if (!match && wantsInteractionContext) {
    match = findDirectInteractionMatch(candidates, event.coordinates, eventMs);
  }
  ...
  const interactionCandidates = wantsInteractionContext
    ? findStableInteractionContainers(match, observations, eventMs, toolStartMs, toolEndMs)
    : [];
  const interactionContainer = interactionCandidates[0];
  return {
    ...event,
    ...
    semanticTarget: {
      ...
      ...(interactionCandidates.length ? { interactionCandidates } : {}),
...
function findDirectInteractionMatch(observations, coordinates, eventMs) {
  const from = coordinates?.from ?? coordinates;
  const to = coordinates?.to ?? coordinates;
  if (![from?.xNorm, from?.yNorm, to?.xNorm, to?.yNorm].every(Number.isFinite)) return undefined;
  const matches = [];
  for (const observation of observations) {
    for (const element of observation.elements ?? []) {
      if (!roleIdentities(element).some(role => INTERACTIVE_ROLES.has(role))) continue;
      const bounds = normalizeBounds(element.bounds, observation.windowBounds);
      if (!bounds || !hasUsableTargetGeometry(
        element.bounds, observation.windowBounds, undefined, element
      )) continue;
      const center = {
        xNorm: bounds.xNorm + bounds.widthNorm / 2,
        yNorm: bounds.yNorm + bounds.heightNorm / 2
      };
      const distance = pointToSegmentDistance(center, from, to);
      const tolerance = Math.max(0.018, Math.hypot(bounds.widthNorm, bounds.heightNorm) / 2 + 0.008);
      if (distance > tolerance) continue;
      matches.push({
        observation,
        element,
        provenance: "ax-spatial-corroboration",
        confidence: geometryConfidence(observation, Math.max(0.76, 0.94 - distance * 4)),
        rank: 10_000 - distance * 100_000
          - Math.abs(Date.parse(observation.observedAt) - eventMs)
          - walkPenalty(observation)
      });
    }
  }
...
function pointToSegmentDistance(point, from, to) {
  const dx = to.xNorm - from.xNorm;
  const dy = to.yNorm - from.yNorm;
  const lengthSquared = dx * dx + dy * dy;
  const phase = lengthSquared > 0
    ? Math.max(0, Math.min(1, ((point.xNorm - from.xNorm) * dx + (point.yNorm - from.yNorm) * dy) / lengthSquared))
    : 0;
  return Math.hypot(
    point.xNorm - (from.xNorm + phase * dx),
    point.yNorm - (from.yNorm + phase * dy)
  );
}

function findStableInteractionContainers(match, observations, eventMs, toolStartMs, toolEndMs) {
  const target = normalizeBounds(match.element.bounds, match.observation.windowBounds);
  if (!target) return [];
  const targetArea = target.widthNorm * target.heightNorm;
  const candidates = (match.observation.elements ?? []).flatMap(element => {
    const bounds = normalizeBounds(element.bounds, match.observation.windowBounds);
    if (!bounds) return [];
    const area = bounds.widthNorm * bounds.heightNorm;
    // Discard control-sized wrappers and near-viewport ancestors. The former
    // add no framing context; the latter are page geometry rather than an
    // interaction surface. These thresholds are normalized geometry, not UI
    // roles or application-specific labels.
    if (area < Math.max(0.008, targetArea * 12) || area > 0.62) return [];
    const contains = containsBounds(bounds, target);
    const gap = boundsGap(bounds, target);
    // Composite widgets commonly place their trigger just outside the visual
    // viewport it changes. Nearby stable geometry therefore remains a
    // candidate, but only when it is materially larger than the control.
    if (!contains && gap > 0.055) return [];
    if (bounds.widthNorm < target.widthNorm * 2.5
        || bounds.heightNorm < target.heightNorm * 2.5) return [];
    const supportingObservations = observations.filter(observation => {
      if (observation === match.observation || observation.treeComplete === false) return false;
      const observedMs = Date.parse(observation.observedAt);
      // Change-driven AX delivery can lag a completed gesture. A short
      // post-transaction verification window establishes persistence without
      // treating an arbitrarily later matching layout as the same surface.
      if (observedMs < eventMs - 30_000 || observedMs > toolEndMs + 5000) return false;
      return (observation.elements ?? []).some(other => {
        const otherBounds = normalizeBounds(other.bounds, observation.windowBounds);
        return otherBounds && boundsSimilarity(bounds, otherBounds) >= 0.92;
      });
    });
    if (!supportingObservations.length) return [];
    return [{
      bounds,
      area,
      contains,
      gap,
      supportCount: supportingObservations.length,
      verifiedAtMs: Math.max(...supportingObservations.map(observation => Date.parse(observation.observedAt)))
    }];
  }).sort((left, right) => {
    const leftScore = Math.log(left.area + 0.001) + left.gap * 3 + (left.contains ? 0.08 : 0);
    const rightScore = Math.log(right.area + 0.001) + right.gap * 3 + (right.contains ? 0.08 : 0);
    return leftScore - rightScore;
  });
  const unique = [];
  for (const candidate of candidates) {
    if (unique.some(existing => boundsSimilarity(existing.bounds, candidate.bounds) >= 0.88)) continue;
    unique.push(candidate);
    if (unique.length === 5) break;
  }
  return unique.map(candidate => ({
    source: "macos-accessibility-stable-container",
    confidence: Math.min(0.96, 0.78 + candidate.supportCount * 0.04),
    relation: candidate.contains ? "contains-trigger" : "adjacent-to-trigger",
    bounds: candidate.bounds,
    verifiedThroughOffsetMs: Math.max(0, candidate.verifiedAtMs - eventMs)
  }));
}

function boundsGap(left, right) {
  const gapX = Math.max(0, left.xNorm - (right.xNorm + right.widthNorm), right.xNorm - (left.xNorm + left.widthNorm));
  const gapY = Math.max(0, left.yNorm - (right.yNorm + right.heightNorm), right.yNorm - (left.yNorm + left.heightNorm));
  return Math.hypot(gapX, gapY);
}

function containsBounds(container, target) {
  const tolerance = 0.003;
  return target.xNorm >= container.xNorm - tolerance
    && target.yNorm >= container.yNorm - tolerance
    && target.xNorm + target.widthNorm <= container.xNorm + container.widthNorm + tolerance
    && target.yNorm + target.heightNorm <= container.yNorm + container.heightNorm + tolerance;
}

function boundsSimilarity(left, right) {
  const x0 = Math.max(left.xNorm, right.xNorm);
  const y0 = Math.max(left.yNorm, right.yNorm);
  const x1 = Math.min(left.xNorm + left.widthNorm, right.xNorm + right.widthNorm);
  const y1 = Math.min(left.yNorm + left.heightNorm, right.yNorm + right.heightNorm);
  const intersection = Math.max(0, x1 - x0) * Math.max(0, y1 - y0);
  const leftArea = left.widthNorm * left.heightNorm;
  const rightArea = right.widthNorm * right.heightNorm;
