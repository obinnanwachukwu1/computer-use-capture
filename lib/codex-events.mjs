...
const ACTIVATABLE_FOCUS_ROLES = new Set([
  "button", "check box", "combo box", "link", "list box", "menu button", "menu item",
  "pop up button", "radio button", "slider", "switch", "tab", "text entry area", "text field"
]);
...
export async function findCodexSessions({
  ...
  for (const candidate of candidates.slice(0, 160)) {
    ...
    try {
      ...
      // Codex's task ID is the strongest available identity and remains
      // stable when macOS canonicalizes a working directory through a symlink
      // (for example /tmp -> /private/tmp). Only fall back to cwd discovery
      // when request metadata did not bind the recorder to an exact task.
      if (threadId) {
        if (candidateThreadId !== threadId) continue;
      } else if (path.resolve(record.payload?.cwd ?? "") !== path.resolve(cwd)) {
        continue;
      }
      matches.push({
        file: candidate.file,
...
export function createComputerUseEventAccumulator({ captureStartedAt }) {
  ...
  function ingest(record) {
    ...
    const postActionFocus = parseFocusedAccessibilityElement(stateText);
    const actionCallIndexes = calls.flatMap((call, index) => {
      const method = METHOD_ALIASES.get(call.method) ?? call.method;
      return CONTEXT_METHODS.has(method) ? [] : [index];
    });
    const finalActionCallIndex = actionCallIndexes.at(-1);
    ...
    calls.forEach((call, index) => {
      ...
      const accessibilityContext = Number.isInteger(elementIndex) && accessibilityState
        ...
        : undefined;
      const exactPostActionFocus = index === finalActionCallIndex
        && postActionFocus?.roleKnown
        && ACTIVATABLE_FOCUS_ROLES.has(postActionFocus.role)
          ? postActionFocus : undefined;
      events.push({
        ...
        ...(exactPostActionFocus ? { postActionFocus: exactPostActionFocus } : {}),
...
export function parseFocusedAccessibilityElement(text) {
  const descriptor = String(text).match(/The focused UI element is\s+(\d+)\s+([^\n]+)/i);
  if (!descriptor) return undefined;
  return parseAccessibilityElements(`${descriptor[1]} ${descriptor[2]}`).get(Number(descriptor[1]));
}
...
export function mergeAccessibilityState(previous, text) {
  ...
  const focused = parseFocusedAccessibilityElement(source);
  if (focused) parsed.set(focused.elementIndex, focused);
