...
import { runtimeBinaryPath, runtimeMode, verifyPrebuiltRuntime } from "./runtime-binaries.mjs";
...
export class RecorderService {
  constructor({
    ...
    storeRoot = process.env.COMPUTER_USE_CAPTURE_STORE
      ?? path.join(os.homedir(), "Library", "Application Support", "ComputerUseCapture", "projects"),
    workingDirectory = process.env.COMPUTER_USE_CAPTURE_WORKING_DIRECTORY ?? process.cwd()
  } = {}) {
  ...
  async start(options = {}, requestContext = {}) {
    const operation = this.startOperation.catch(() => {}).then(() => this.#startInternal(options, requestContext));
    this.startOperation = operation;
    return operation;
  }

  async #startInternal(
    { app = "com.apple.Safari", maxDurationSeconds = 1800, label, idempotencyKey, threadId } = {},
    requestContext = {}
  ) {
    const workingDirectory = this.#workingDirectory(requestContext);
    if (!isBundleIdentifier(app)) {
      throw new RecorderError("invalid_argument", "app must be a macOS bundle identifier");
    }
    ...
    const preflight = await this.#preflight(app);
    ...
    if (!target?.available) {
      throw new RecorderError(
        ...
        target?.windowCount > 1
          ? `Exactly one eligible window for ${app} is required; found ${target.windowCount}`
          : `No eligible on-screen window for ${app} is available`,
        { data: { windowCount: target?.windowCount ?? 0 } }
      );
    }
    const adapterProbe = (await this.#adapterProbes(app, workingDirectory, threadId)).codexRollout;
    ...
    const child = spawn(process.execPath, [
      path.join(this.repoRoot, "scripts", "record-session.mjs"), base, String(maxDurationSeconds), app
    ], {
      cwd: workingDirectory,
      env: {
        ...process.env,
        COMPUTER_USE_CAPTURE_PERSIST_PRIVATE_PATHS: "0",
        COMPUTER_USE_CAPTURE_PARENT_PID: String(process.pid),
        COMPUTER_USE_CAPTURE_CAPTURE_MODE: captureModeFor(
          app, process.env.COMPUTER_USE_CAPTURE_CAPTURE_MODE
        ),
        ...(threadId ? { COMPUTER_USE_CAPTURE_THREAD_ID: threadId } : {})
      },
    ...
    const mode = captureLine?.match(/mode=([^\s]+)/)?.[1];
    manifest = {
      ...manifest,
      state: "recording",
      startedAt,
  ...
  async edit({ recordingId, intents = {}, review }) {
    return this.#withRecordingOperation(recordingId, () => this.#editInternal({ recordingId, intents, review }));
  }

  async #editInternal({ recordingId, intents, review }) {
    let manifest = await this.#requireRecording(recordingId);
    if (manifest.state !== "stopped") throw new RecorderError("invalid_state", `Recording is ${manifest.state}, not stopped`);
    const normalizedReview = await this.#requireCompletedRenderReview(manifest, review);
    ...
    let effectiveIntents = mergeIntents(manifest.effectiveIntents, intents);
    await validateRenderAssets(effectiveIntents);
    effectiveIntents = await this.#importRenderAssets(recordingId, effectiveIntents);
    const notes = await this.#applyTimelineIntents(manifest, effectiveIntents);
    manifest = { ...manifest, effectiveIntents };
    await this.#writeManifest(manifest);
    const renderJob = await this.#startRender(manifest, effectiveIntents, normalizedReview);
    return {
      recordingId, renderId: renderJob.renderId, state: "queued",
      effectiveIntents, review: normalizedReview, notes
    };
  }
  ...
  async capabilities(_options = {}, requestContext = {}) {
    const workingDirectory = this.#workingDirectory(requestContext);
    await this.#ensureBinaries();
    const active = await this.#activeRecording();
  ...
  async activeRecordingId() {
    return (await this.#activeRecording())?.recordingId ?? null;
  }

  async hasActiveJobs() {
    return this.recordingProcesses.size > 0 || this.renderProcesses.size > 0;
  }
  ...
  async #startRender(manifest, intents, review) {
    ...
    if (intents.background?.type === "image") args.push("--wallpaper", intents.background.path);
    if (intents.background?.type === "solid") args.push("--background-color", intents.background.color);
    if (intents.cursor?.asset?.type === "image") {
      args.push("--cursor", intents.cursor.asset.path, "--cursor-metadata", intents.cursor.asset.metadataPath);
    }
    const env = {
      ...process.env,
      COMPUTER_USE_CAPTURE_PARENT_PID: String(process.pid),
    ...
    const tracked = trackChild(child, path.join(this.#projectDir(manifest.recordingId), "renders", `${renderId}.log`), line => {
      let payload;
      if (line.startsWith("COMPUTER_USE_CAPTURE_PROGRESS ")) {
        try { payload = JSON.parse(line.slice("COMPUTER_USE_CAPTURE_PROGRESS ".length)); } catch { /* malformed progress is non-fatal */ }
      }
  ...
  async #requireCompletedRenderReview(manifest, review) {
    const reason = typeof review?.reason === "string" ? review.reason.trim() : "";
    if (!review?.sourceRenderId || reason.length < 12) {
      throw new RecorderError(
        "invalid_argument",
        "recorder_edit requires a completed render that was inspected and a specific review reason"
      );
    }
    const source = manifest.renders.find(candidate => candidate.renderId === review.sourceRenderId);
    if (!source || source.state !== "completed" || !await exists(source.output)) {
      throw new RecorderError(
        "invalid_argument",
        "review.sourceRenderId must identify a completed, inspectable render from this recording"
      );
    }
    return { sourceRenderId: source.renderId, reason };
  }
  ...
  async #renderExited(recordingId, renderId, exit) {
    ...
    const cameraAudit = await readJson(current.output.replace(/\.mp4$/, ".camera-audit.json")).catch(() => undefined);
    ...
    await this.#updateRender(recordingId, renderId, {
      ...
      quality: renderQuality(report, cameraAudit)
    });
  }
  ...
  async #applyTimelineIntents(manifest, intents) {
    ...
    timeline.composition.director.deadTimeRate = intents.waiting?.motionRate ?? 2;
  ...
  async #recordingResult(manifest) {
    ...
    return {
      ...
      artifacts: timeline ? {
        ...
        frameProvenance: await artifact(`${manifest.base}.capture.json`)
      } : undefined,
  ...
  async #preflight(app) {
    const binary = runtimeBinaryPath(this.repoRoot, "recorder-preflight");
    if (!await exists(binary)) return {};
    const result = await runToCompletion(binary, app ? [app] : [], { cwd: this.repoRoot, timeoutMs: 5000 }).catch(() => undefined);
    return result?.code === 0 ? JSON.parse(result.stdout) : {};
  }

  async #adapterProbes(app, workingDirectory = this.workingDirectory, threadId) {
    const sessions = await findCodexSessions({ cwd: workingDirectory, threadId }).catch(() => []);
    const screenshotAppName = app?.split(".").at(-1);
    const screenshot = screenshotAppName ? await findScreenshotCoordinateSpace({
      appName: screenshotAppName, referenceTime: Date.now()
    ...
    return {
      codexRollout: rolloutProbe ? {
        ...
        status: "unavailable",
        detail: `No Codex task found for ${workingDirectory}`
      },
      screenshotCache: screenshot ? {
        ...
        detail: screenshot.confidence === "stale"
          ...
          : `Current ${screenshotAppName} screenshot evidence is readable (${screenshot.width}x${screenshot.height}); capture compatibility is validated per action`
      } : {
        status: "unavailable",
        detail: app
          ? `No Computer Use screenshot evidence is currently available for ${screenshotAppName}`
          : "Screenshot evidence is checked for the selected app when recording starts"
      }
    };
  }

  #workingDirectory(requestContext) {
    const candidate = requestContext?.workingDirectory;
  ...
  async #ensureBinaries() {
    if (!this.buildPromise) {
      this.buildPromise = (runtimeMode(this.repoRoot) === "prebuilt"
        ? verifyPrebuiltRuntime(this.repoRoot)
        : runToCompletion("swift", ["build", "-c", "release"], {
  ...
  async #importRenderAssets(recordingId, intents) {
    const assetsDir = path.join(this.#projectDir(recordingId), "assets");
    await mkdir(assetsDir, { recursive: true, mode: 0o700 });
    const importAsset = async (source, stem) => {
      if (path.dirname(path.resolve(source)) === assetsDir) return path.resolve(source);
      const extension = safeAssetExtension(source);
      const destination = path.join(assetsDir, `${stem}${extension}`);
      await copyFile(source, destination);
      await chmod(destination, 0o600);
      return destination;
    };
    const background = intents.background?.type === "image"
      ? { ...intents.background, path: await importAsset(intents.background.path, "background") }
      : intents.background;
    const cursorAsset = intents.cursor?.asset?.type === "image"
      ? {
          ...intents.cursor.asset,
          path: await importAsset(intents.cursor.asset.path, "cursor"),
          metadataPath: await importAsset(intents.cursor.asset.metadataPath, "cursor-metadata")
        }
      : intents.cursor?.asset;
    return {
      ...intents,
      background,
      cursor: { ...intents.cursor, asset: cursorAsset }
    };
  }
...
export function captureModeFor(_app, override) {
  return override ?? "window-crop";
}
...
function mergeIntents(current, patch) {
  return {
    ...
    background: patch.background ?? current.background ?? { type: "system-wallpaper" },
    cursor: {
      ...current.cursor, ...patch.cursor,
      asset: patch.cursor?.asset ?? current.cursor?.asset ?? { type: "system" }
    },
    actions: patch.actions ?? current.actions
  };
}

async function validateRenderAssets(intents) {
  const paths = [];
  if (intents.background?.type === "image") paths.push(intents.background.path);
  if (intents.cursor?.asset?.type === "image") {
    paths.push(intents.cursor.asset.path, intents.cursor.asset.metadataPath);
  }
  for (const file of paths) {
    if (typeof file !== "string" || !path.isAbsolute(file)) {
      throw new RecorderError("invalid_argument", "Custom cursor and wallpaper paths must be absolute");
    }
    await access(file).catch(() => {
      throw new RecorderError("invalid_argument", `Custom render asset does not exist: ${file}`);
    });
  }
}
...
export function renderQuality(report, cameraAudit) {
  const issues = [];
  const unresolved = report?.pointerRendering?.omittedUnresolved ?? 0;
  const unframed = cameraAudit?.semanticCoverage?.unframedSustainedResponses ?? 0;
  const causalOrderViolations = cameraAudit?.causalOrdering?.violations ?? 0;
  const baseUpscale = report?.sourceRaster?.baseUpscaleFactor ?? 1;
  if (cameraAudit?.planFeasible === false) {
    issues.push(`camera planner failed: ${cameraAudit.planFailure ?? "no globally feasible camera plan"}`);
  }
  if (unresolved > 0) issues.push(`${unresolved} factual pointer action${unresolved === 1 ? " was" : "s were"} unresolved`);
  if (unframed > 0) issues.push(`${unframed} sustained visual response${unframed === 1 ? " was" : "s were"} not tightly framed`);
  if (causalOrderViolations > 0) {
    issues.push(`${causalOrderViolations} action${causalOrderViolations === 1 ? " was" : "s were"} rendered after its visual response began`);
  }
  if (baseUpscale > 1.000001) {
    issues.push(`source raster was enlarged ${baseUpscale.toFixed(2)}× before camera zoom`);
  }
...
function motionBlurEnvironment(value) {
  if (value === "none") return { COMPUTER_USE_CAPTURE_MOTION_SAMPLES: "1", COMPUTER_USE_CAPTURE_SHUTTER: "0" };
  if (value === "strong") return { COMPUTER_USE_CAPTURE_MOTION_SAMPLES: "10", COMPUTER_USE_CAPTURE_SHUTTER: "0.75" };
  return { COMPUTER_USE_CAPTURE_MOTION_SAMPLES: "8", COMPUTER_USE_CAPTURE_SHUTTER: "0.55" };
}
