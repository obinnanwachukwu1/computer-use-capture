...
export function runtimeMode(repoRoot) {
  const explicit = process.env.AGENTRECORDER_BINARY_MODE;
  if (explicit && !["source", "prebuilt"].includes(explicit)) {
    throw new Error("AGENTRECORDER_BINARY_MODE must be source or prebuilt");
  }
  if (process.env.AGENTRECORDER_BINARY_DIR) return "prebuilt";
  if (explicit) return explicit;
  return existsSync(path.join(repoRoot, ".git")) ? "source" : "prebuilt";
}

export function runtimeBinaryDirectory(repoRoot) {
  if (process.env.AGENTRECORDER_BINARY_DIR) {
    return path.resolve(process.env.AGENTRECORDER_BINARY_DIR);
  }
