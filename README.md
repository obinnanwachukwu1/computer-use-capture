...
The production camera planner is the normal, unnamed path. It preserves alternative interaction timings, causal attributions, foreground lifecycles, and attention regions in an immutable graph, then jointly selects timing, activation framing, response framing, and the camera trajectory across the entire clip. It has no shot or episode reset states; factual cursor visibility and Computer Use event ordering are hard constraints, while movement and editorial taste remain costs. The same selected action times feed a single global retime partition. Long keyboard-only intervals use an explicit pointer-reveal transition when the synthetic cursor's factual departure lies outside the held shot. Each run writes a `.production-plan.json` decision and search trace beside the camera audit. See [`docs/production-planner.md`](docs/production-planner.md).

The subject-first replacement is explicitly experimental. `SubjectGraph` first infers persistent surfaces, factual targets, scene transitions, and localized responses without prescribing camera behavior. `ShotSchedule` then prices emphasis once per subject span and emits explicit overview, framing, orientation, and response beats. It is reachable only through `--experimental-camera-planner` (or `AGENTRECORDER_EXPERIMENTAL_CAMERA_PLANNER=1`), writes `.experimental-plan.json`, and is never selected by the MCP default. See [`docs/experimental-camera-planner.md`](docs/experimental-camera-planner.md).
...
npm run benchmark:pipeline -- \
  ...
  --mode both --trials 3 --planner normal --samples 8 \
  --analysis-cache warm
```
